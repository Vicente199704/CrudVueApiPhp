<template>
  <div class="container">
        <router-link to="/crear" class="btn btn-success">Agregar Un Nuevo Empleado</router-link> 
    <br>    <br>
    <div class="card">
...
export default {
  ...
  methods: {
    ...
    consultarEmpleados() {
      fetch("http://localhost/empleados/") //Consultando un XAMP
    ...
    borrarEmpleado(id){
        console.log(id)
         fetch('http://localhost/empleados/?borrar='+id) //Consultando un XAMP
        .then((respuesta) => respuesta.json())
        .then((datosRespuesta) => {
          console.log(datosRespuesta);
